...
create type foodie.reminder_kind as enum ('scheduled', 'event');

-- What a reminder is about — used for grouping and per-category preferences.
create type foodie.notification_category as enum
...
create type foodie.reminder_event_type as enum
  ('component_replacement_due', 'fermentation_check_due', 'starter_feed_due',
   'inventory_low', 'food_expiring', 'supply_low', 'custom');

create type foodie.notification_status as enum
  ('pending', 'sent', 'partially_sent', 'failed', 'canceled');

create type foodie.delivery_channel as enum ('push', 'sms', 'email', 'dashboard');

create type foodie.delivery_status as enum ('pending', 'sent', 'failed', 'skipped');

-- ---------------------------------------------------------------------------
-- reminder_rules
-- ---------------------------------------------------------------------------
create table foodie.reminder_rules (
  ...
  category           foodie.notification_category not null default 'custom',
  kind               foodie.reminder_kind not null,
...
create index idx_reminder_rules_household
  on foodie.reminder_rules (household_id) where deleted_at is null;
create index idx_reminder_rules_next_run
  on foodie.reminder_rules (next_run_at)
  where enabled and deleted_at is null;

-- ---------------------------------------------------------------------------
-- notifications — one generated, channel-neutral message instance.
-- ---------------------------------------------------------------------------
create table foodie.notifications (
  id                 uuid primary key default gen_random_uuid(),
  household_id       uuid not null references foodie.households (id) on delete cascade,
  reminder_rule_id   uuid references foodie.reminder_rules (id) on delete set null,
  recipient_user_id  uuid references foodie.profiles (id) on delete cascade,  -- null = all members
  category           foodie.notification_category not null default 'custom',
  title              text not null,
  body               text not null,
  ...
  status             foodie.notification_status not null default 'pending',
  created_at         timestamptz not null default now(),
  updated_at         timestamptz not null default now(),
...
create index idx_notifications_pending
  on foodie.notifications (scheduled_for) where status = 'pending';
create index idx_notifications_household
  on foodie.notifications (household_id, created_at desc);

-- ---------------------------------------------------------------------------
-- notification_deliveries — one row per channel attempt. Twilio is just a
-- provider value on an sms-channel row.
-- ---------------------------------------------------------------------------
create table foodie.notification_deliveries (
  id                  uuid primary key default gen_random_uuid(),
  household_id        uuid not null references foodie.households (id) on delete cascade,
  notification_id     uuid not null references foodie.notifications (id) on delete cascade,
  recipient_user_id   uuid references foodie.profiles (id) on delete cascade,
  channel             foodie.delivery_channel not null,
  provider            text,                      -- 'twilio', 'apns', ... set by the worker
  status              foodie.delivery_status not null default 'pending',
...
create trigger trg_reminder_rules_updated_at           before update on foodie.reminder_rules           for each row execute function foodie.set_updated_at();
create trigger trg_notifications_updated_at            before update on foodie.notifications            for each row execute function foodie.set_updated_at();
create trigger trg_notification_deliveries_updated_at  before update on foodie.notification_deliveries  for each row execute function foodie.set_updated_at();
create trigger trg_notification_preferences_updated_at before update on foodie.notification_preferences for each row execute function foodie.set_updated_at();
...
-- ---------------------------------------------------------------------------
alter table foodie.reminder_rules           enable row level security;
alter table foodie.notifications            enable row level security;
alter table foodie.notification_deliveries  enable row level security;
alter table foodie.notification_preferences enable row level security;

create policy reminder_rules_member_all on foodie.reminder_rules
  for all using (foodie.is_household_member(household_id))
  with check (foodie.is_household_member(household_id));

create policy notifications_member_select on foodie.notifications
  for select using (foodie.is_household_member(household_id));
create policy notifications_member_update on foodie.notifications
  for update using (foodie.is_household_member(household_id))
  with check (foodie.is_household_member(household_id));

create policy notification_deliveries_member_select on foodie.notification_deliveries
  for select using (foodie.is_household_member(household_id));

create policy notification_preferences_own_all on foodie.notification_preferences
  for all using (user_id = auth.uid() and foodie.is_household_member(household_id))
  with check (user_id = auth.uid() and foodie.is_household_member(household_id));
create policy notification_preferences_member_select on foodie.notification_preferences
  for select using (foodie.is_household_member(household_id));
