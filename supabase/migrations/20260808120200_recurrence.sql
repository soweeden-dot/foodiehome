...
create type foodie.interval_unit as enum ('day', 'week', 'month', 'year');

create table foodie.recurrence_rules (
  ...
  interval_unit  foodie.interval_unit not null,
...
create index idx_recurrence_rules_household on foodie.recurrence_rules (household_id);

create trigger trg_recurrence_rules_updated_at
  before update on foodie.recurrence_rules
  for each row execute function foodie.set_updated_at();

alter table foodie.recurrence_rules enable row level security;

create policy recurrence_rules_member_all on foodie.recurrence_rules
