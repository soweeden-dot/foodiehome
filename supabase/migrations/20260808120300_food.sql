...
create type public.supply_level as enum
  ('full', 'good', 'low', 'almost_empty', 'out');

create type public.meal_slot as enum ('breakfast', 'lunch', 'dinner', 'snack', 'prep');
create type public.meal_entry_status as enum ('planned', 'prepped', 'cooked', 'skipped');
...
create index idx_inventory_items_food_item on public.inventory_items (food_item_id);
...
create index idx_recipe_ingredients_food_item on public.recipe_ingredients (food_item_id);
...
create table public.meal_plan_entries (
  ...
  status        public.meal_entry_status not null default 'planned',
  notes         text,
  created_at    timestamptz not null default now(),
  updated_at    timestamptz not null default now(),
  deleted_at    timestamptz,
...
create index idx_grocery_items_source_ref on public.grocery_items (source_ref)
...
create trigger trg_recipe_steps_updated_at        before update on public.recipe_steps        for each row execute function public.set_updated_at();
create trigger trg_recipe_ingredients_updated_at  before update on public.recipe_ingredients  for each row execute function public.set_updated_at();
...
create trigger trg_meal_plan_entries_updated_at   before update on public.meal_plan_entries   for each row execute function public.set_updated_at();
create trigger trg_grocery_lists_updated_at       before update on public.grocery_lists       for each row execute function public.set_updated_at();
...
create policy inventory_locations_member_all on public.inventory_locations
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy food_items_member_all on public.food_items
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy inventory_items_member_all on public.inventory_items
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy recipes_member_all on public.recipes
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy recipe_steps_member_all on public.recipe_steps
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy recipe_ingredients_member_all on public.recipe_ingredients
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy meal_plans_member_all on public.meal_plans
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy meal_plan_entries_member_all on public.meal_plan_entries
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy grocery_lists_member_all on public.grocery_lists
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
create policy grocery_items_member_all on public.grocery_items
  for all using (public.is_household_member(household_id)) with check (public.is_household_member(household_id));
